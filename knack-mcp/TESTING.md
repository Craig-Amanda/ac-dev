...
| ----- | ------------ | ------------------------------------------------------------------------------------------------ |
| 1     | P1           | One builder capture that decides whether repointing column connections is our job or Knack's     |
...
| ID  | Case                                                                                                                                                                                              | Expected                                                                                                        | Proven                                                                                                                                                                                                                     |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P1  | **In the builder, change a view's source connection on a view with connection-reached columns, then capture the save request.** Does the builder rewrite `columns[].connection.key`, or leave it? | Unknown, and it decides whether repointing those is our job or Knack's                                          | **—**. The highest-value capture available. If the builder rewrites them, our tools must too or a repoint is incomplete; if it leaves them, they are broken by design and the plan tool is the only thing that will say so |
| P2  | Same question for `columns[].edit_rules[].connection`, the dotted `object_N.field_N` form                                                                                                         | Follows P1, or does not — the dotted form may be handled separately                                             | **—**                                                                                                                                                                                                                      |
| P3  | Repoint a view whose columns reach through the old connection, leave the column keys stale, and read the rendered rows                                                                            | Columns render values from the wrong relationship rather than erroring — the failure mode being guarded against | **—**. Predicted from the shapes, never observed. If it errors instead, the hazard is smaller than claimed and `knack_plan_view_repoint`'s warning should be softened                                                      |
| P4  | `knack_plan_view_repoint` against a real view with connection columns                                                                                                                             | Reports every connection reference, its path, and the distinct connection fields                                | **automated** against a reduction of a real copy request; **—** live. Reverting any of its four rules fails the suite                                                                                                      |
...
Most useful next, in order: **P1's before-and-after pair** (one view, source connection
changed, both save requests); a **`move_view`** request, since C8's move arm has never
been observed; a **details or list** view with connection-reached columns, to see whether
the column shape differs from a table's; and a **form** with connection inputs and submit
rules.
