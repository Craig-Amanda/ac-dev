...
| Cutting one of **three** links transfers the page too, so the rule is not a two-link special case | **live, twice**, 7 Sep — both children survived. Both landed on the same candidate page with the alternate-route creation order reversed between runs, which eliminates link creation order and view key order but not scene order or lowest scene key: the two runs shared their candidate pair. The destination is measured, not predicted — `TESTING.md` Tier 6 |
