...
| Cutting one of **three** links transfers the page too, so the rule is not a two-link special case               | **live, three times**, 7 Sep — every child survived                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| A transferred page lands on the surviving referrer that comes **first in the order Knack returns its pages in** | **live, three times**, 7 Sep, the third **pre-registered**. Runs 1–2 reversed the alternate-route creation order, eliminating link creation order and view key order. Run 3 ran across a pair where returned page order and numeric key order disagree, with both outcomes written down beforehand: page order predicted one page, lowest key the other, and page order won. Page order 3/3; **lowest numeric scene key eliminated**. Named in the confirmation prompt and in `knack_list_page_referrers`, hedged in both — three observations, and the order it keys off is one a builder edit can change. `knack-mcp-v2/TESTING.md` Tier 6 |
