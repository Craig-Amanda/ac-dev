...
const ENV_MAX_EXTRACTED_TEXT_BYTES = process.env.KNACK_MCP_MAX_EXTRACTED_TEXT_BYTES;
...
const MAX_EXTRACTED_TEXT_BYTES = getPositiveIntEnv(ENV_MAX_EXTRACTED_TEXT_BYTES, DEFAULT_MAX_EXTRACTED_TEXT_BYTES);
...
function createServer(options: ServerOptions = {}) {
     ...
     */
    async function getRecordAttachment(app: AppConfig, objectKey: string, recordId: string, fieldKey: string) {
        ...
        if (!field || !['file', 'image'].includes(field.type || '')) {
            throw new Error(`Field ${fieldKey} is not a file or image field on ${objectKey}.`);
        }
        ...
        if (!attachment || !url || !filename) {
            throw new Error(`Field ${fieldKey} does not contain an uploaded attachment.`);
        }

        return {
            url,
            filename,
            mimeType: typeof attachment.mime_type === 'string' ? attachment.mime_type : 'application/octet-stream',
    ...
    async function downloadRecordAttachment(
        app: AppConfig,
        recordId: string,
        attachment: { url: string; filename: string; mimeType: string; sizeBytes: number | null },
     ...
     */
    async function extractAttachmentText(filePath: string, mimeType: string): Promise<{ text: string; truncated: boolean; supported: boolean }> {
    ...
    server.tool(
        ...
        {
            appKey: z.string().optional(),
            objectKey: z.string().describe('Object containing the file or image field.'),
            recordId: z.string().describe('Record containing the attachment.'),
            fieldKey: z.string().describe('File or image field containing the attachment.'),
        },
        async ({ appKey, objectKey, recordId, fieldKey }) => {
            const app = getAppOrThrow(appKey);
            debugLog('tool_call', { tool: 'knack_download_file', args: { appKey: app.appKey, objectKey, recordId, fieldKey } });
            const attachment = await getRecordAttachment(app, objectKey, recordId, fieldKey);
            const download = await downloadRecordAttachment(app, recordId, attachment);
    ...
    server.tool(
        ...
        async ({ appKey, objectKey, recordId, fieldKey }) => {
            const app = getAppOrThrow(appKey);
            debugLog('tool_call', { tool: 'knack_read_file', args: { appKey: app.appKey, objectKey, recordId, fieldKey } });
            const attachment = await getRecordAttachment(app, objectKey, recordId, fieldKey);
            const download = await downloadRecordAttachment(app, recordId, attachment);
            const extraction = await extractAttachmentText(download.filePath, attachment.mimeType);
            ...
            return makeTextResponse({
                ...
                message: extraction.supported ? null : 'The file was downloaded, but its format is not supported for text extraction.',
            });
        }
    );
