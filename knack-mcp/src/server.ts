    ...
    return {
      ...
      sample:
        depth >= 2
          ...
          : value
              .slice(0, 3)
              .map((entry) => summariseLargeValue(entry, depth + 1)),
    };
  }
  ...
  const sampleEntries =
    depth >= 2
      ...
      : Object.fromEntries(
          keys
            .slice(0, 8)
            .map((key) => [key, summariseLargeValue(record[key], depth + 1)]),
        );
...
function buildTemplateFieldDescriptors(
  ...
  const selectedFields =
    fieldKeys.length > 0
      ? fieldKeys.map(
          (fieldKey) => objectFieldsByKey.get(fieldKey) || { key: fieldKey },
        )
  ...
  for (const [key, value] of Object.entries(node as Record<string, unknown>)) {
    ...
    if (/^field_\d+$/i.test(key)) {
      ...
      addFieldReference(context.index, context.dedupe, {
        ...
        classification: [
          ...classifyFieldReference(context.sourceType, nextPathParts),
          "propertyKey",
        ],
        containingText: null,
        objectKey: context.objectKey,
...
const KNACK_FIELD_SHAPES: Record<string, FieldShapeInfo> = {
  ...
  paragraph_text: {
    ...
    rawShape: '"Line one\\nLine two"',
    notes:
      "Formatted output can contain HTML line breaks. Raw preserves newline characters.",
  },
  email: {
    ...
    notes:
      "Formatted output is typically a mailto anchor. Raw is an object with email and label.",
  },
  phone: {
    ...
    notes:
      "Formatted output is typically a tel anchor. Raw is an object containing number parts and preformatted variants.",
  },
  number: {
    ...
    rawShape: 1234.5,
    notes:
      "Raw is a JS number. Formatted output depends on the field display settings and may include prefixes or suffixes.",
  },
  currency: {
    ...
    rawShape: '"1234.56"',
    notes:
      "Formatted includes currency symbols and separators. Raw is commonly a numeric string rather than a JS number.",
  },
  auto_increment: {
  ...
  equation: {
    ...
    notes:
      "Equation fields can return numbers, plain strings, or date-like values depending on configuration. For date-returning equations, raw may be a scalar date string or a structured date object, while formatted applies the field display format.",
  },
  sum: {
  ...
  name: {
    ...
    notes:
      "Raw is an object with individual name parts. Optional keys such as middle or suffix may be omitted or blank.",
  },
  address: {
    ...
    notes:
      "Formatted output can contain HTML line breaks. Raw includes address components plus a full string; geo fields are often null.",
  },
  date_time: {
    ...
    notes:
      "Formatted output depends on the field configuration and may be date-only, time-only, or a range. Raw for native date/time fields is typically a structured object with date/time parts, proper_* timestamp keys, and an optional to object for ranges rather than a scalar string.",
  },
  timer: {
    ...
    notes:
      "Formatted is human-readable elapsed time. Raw contains an array of from/to time pairs plus totals.",
  },
  multiple_choice: {
    ...
    notes:
      "Raw is a string for single-select controls and an array for multi-select controls. Formatted is a display string.",
  },
  connection: {
    ...
    notes:
      "Raw is an array of objects with id and identifier. Formatted output is HTML, usually one span per connected record, not a plain comma-joined string.",
  },
  file: {
  ...
  image: {
    ...
    notes:
      "Raw includes full-size and thumbnail URLs. Formatted is an HTML img tag.",
  },
  signature: {
    ...
    notes:
      "Observed raw payload contains SVG markup plus a base30 stroke encoding rather than hosted image URLs or timestamp metadata.",
  },
  link: {
  ...
  password: {
    ...
    notes:
      "Knack never returns the password value. Raw only indicates validation strength.",
  },
};
...
function findFieldByName(
  ...
  return fields.find(
    (field) => getFieldHeader(field).trim().toLowerCase() === normalizedTarget,
  );
}
...
function chooseUniqueImportField(
  ...
  return fields.find((field) => {
    ...
    return (
      !["connection", "multiple_choice", "address", "name"].includes(type) &&
    ...
    case "connection": {
      ...
      if (!connectedMeta && externalLookup?.values.length) {
        const selectedValues = [
          externalLookup.values[rowIndex % externalLookup.values.length],
        ];
        if (
...
function validateFieldShape(
  ...
  switch (fieldType.toLowerCase()) {
    ...
    case "name":
      ...
      addFinding(
        rawHasKeys(raw, ["first", "last", "full", "title", "middle", "suffix"]),
    ...
    case "image":
      ...
      addFinding(
        rawHasKeys(raw, ["id", "filename", "url", "thumb_url", "mime_type"]),
...
function createServer(options: ServerOptions = {}) {
  ...
  function inferAppKeyFromPath(contextPath: string): AppInferenceResult {
    ...
    const segmentMatches = apps.filter((app) =>
      getAppAliases(app).some((alias) => normalisedSegments.includes(alias)),
    );
  ...
  async function getRecordAttachment(
    ...
    const { object } = await getPermittedReadFields(app, objectKey, [fieldKey]);
    ...
    if (!result.ok || !record) {
      throw new Error(`Unable to fetch record ${recordId} from ${objectKey}.`);
    }
  ...
  async function downloadRecordAttachment(
    ...
    const safeFilename =
      ...
      "attachment";
    const safeAppKey =
      app.appKey.replace(/[^a-zA-Z0-9._-]/g, "_") || "app";
    const safeRecordId =
      ...
      const download = (url: URL, redirectsRemaining: number) => {
        const request = https.get(url, (response) => {
          ...
          if (statusCode < 200 || statusCode >= 300) {
            response.resume();
            reject(
              new Error(`Attachment download failed with HTTP ${statusCode}.`),
            );
            return;
          }
          if (contentLength && contentLength > MAX_RESPONSE_BYTES) {
        ...
        request.setTimeout(30_000, () =>
          request.destroy(
            new Error("Attachment download timed out after 30 seconds."),
          ),
        );
  ...
  async function extractAttachmentText(
    ...
    if (mimeType === "application/pdf" || extension === ".pdf") {
      ...
      ["application/json", "application/xml", "text/csv"].includes(mimeType) ||
  ...
  function resolveMetadataFilePath(app: AppConfig, fileName: string): string {
    ...
    return (
      candidates.find((candidate) => fileExists(candidate)) || candidates[0]
    );
  }
  ...
  async function getFieldMapForApp(
    app: AppConfig,
  ): Promise<{ fieldMap: CachedFieldMap | null; source: CacheSource | null }> {
  ...
  async function getFieldReferenceIndexForApp(
    app: AppConfig,
  ): Promise<{
  ...
  async function getPermittedReadFields(
    ...
    for (const fieldKey of fields) {
      if (!knownFields.has(fieldKey))
        throw new Error(`Field ${fieldKey} does not belong to ${objectKey}.`);
  ...
  async function applyRecordReadPolicy(
    ...
    const defaultFields = (object?.fields || [])
      ...
      .filter(
        (fieldKey) => !app.dataAccess?.redactedFieldKeys?.includes(fieldKey),
      );
  ...
  server.tool(
    ...
    "Set the active Knack app using either an explicit appKey or a file/folder path. The server can infer the app from KnackApps paths and common app-name aliases.",
    {
      appKey: z
        .string()
        .optional()
        .describe("Explicit app key to activate immediately, e.g. GAP-Track."),
      contextPath: z
        .string()
        .optional()
        .describe(
          ...
          try {
            ...
            if (persistFiles) {
              ...
              if (fieldReferenceResult.index) {
                persisted.fieldReferenceIndex = writeMetadataJson(
                  app,
                  "fieldReferenceIndex.json",
  ...
  server.tool(
    ...
    async ({
      ...
      if (direction === "forward") {
        ...
        if (sortField)
          await getPermittedReadFields(app, targetObjectKey, [sortField]);
        ...
        for (const record of records) {
          ...
          if (dateBucket)
            dimensions[dateBucket.fieldKey] =
              bucketDate(record[dateBucket.fieldKey], dateBucket.granularity) ||
              "Unknown";
          ...
          for (const metric of metrics) {
            const metricKey =
              metric.type === "count" ? "count" : `sum:${metric.fieldKey}`;
            ...
            if (metric.type === "count")
              ...
              if (numeric !== null)
                values[metricKey] = (values[metricKey] || 0) + numeric;
            }
          }
  ...
  server.tool(
    ...
    async ({ appKey, mappingObject }) => {
      ...
      debugLog("tool_call", {
        ...
        args: { appKey, mappingSize: Object.keys(mappingObject).length },
      });
  ...
  server.tool(
    ...
    async ({ appKey, mappingObject }) => {
      ...
      debugLog("tool_call", {
        ...
        args: {
          appKey,
          mappingSize: mappingObject ? Object.keys(mappingObject).length : 0,
        },
      });
  ...
  server.tool(
    ...
    "Return the field type for a field key or alias from schema data.",
    {
      ...
      objectKey: z
        .string()
        .optional()
        .describe("Optional object key to scope the lookup, e.g. object_2"),
    },
  ...
  server.tool(
    ...
    async ({ appKey, objectKey }) => {
      ...
      const typeSummary = [...typeCounts.entries()]
        ...
        .sort(
          (a, b) => b.count - a.count || a.fieldType.localeCompare(b.fieldType),
        );
  ...
  server.tool(
    ...
    async ({ appKey, fieldKey, maxResults }) => {
      ...
      const results = [...viewsByKey.values()].map((entry) => ({
        ...entry,
        matchedPaths: [...new Set(entry.matchedPaths)].sort((left, right) =>
  ...
  server.tool(
    ...
    async ({ appKey, fieldKey, maxResults }) => {
      ...
      return makeTextResponse({
        ...
        totalReferences:
          fieldReferenceResult.index?.[normalisedFieldKey]?.length || 0,
  ...
  server.tool(
    ...
    async ({ appKey, keyword, maxResults }) => {
      ...
      for (const [viewKey, viewAttrs] of Object.entries(viewMap)) {
        ...
        const titleHits = extractKtlKeywordsFromText(title).map((entry) => ({
          ...entry,
          source: "title",
        }));
  ...
  if (HAS_DIAGNOSTIC_TOOLS) {
    server.tool(
      ...
      async ({ appKey, objectKey, recordId, includeBlankFields = false }) => {
        ...
        const schema = schemaResult.schema;
        const obj =
          schema?.objects?.find((entry) => entry.key === objectKey) || null;
        ...
        for (const cf of connections) {
          if (cf.connectedObject) {
            relationships.push({
              ...
              toObjectName:
                objectKeyToName.get(cf.connectedObject) || cf.connectedObject,
            });
          }
        }
  ...
  server.tool(
    ...
    "Generate Knack import-ready seed CSV content for new-object imports. Produces one CSV per object with headers, realistic example rows, connection lookup values that match generated parent rows, and comma-separated multi-select values. If you opt into API-backed existing parent lookups, the tool first returns a rough API-call estimate and requires explicit confirmation before using the API key.",
    {
      ...
      rowsPerObject: z
        ...
        .max(10)
        .default(4)
        .describe("Minimum number of example rows to generate per object."),
  ...
  if (HAS_MUTATION_TOOLS) {
    ...
    server.tool(
      ...
      async ({ appKey, objectKey, sourceFieldKey, newName }) => {
        ...
        debugLog("tool_call", {
          ...
          args: { appKey: app.appKey, objectKey, sourceFieldKey, newName },
        });
    ...
    server.tool(
      ...
      "Create a new record in a Knack object. Requires readonly: false.",
      {
        ...
        data: z
          .string()
          .describe("Record data as JSON string with field_key: value pairs"),
      },
  ...
  if (HAS_VIEW_MUTATION_TOOLS) {
            ...
            if (linkColumns.length > 0) {
              return makeTextResponse({
                ...
                linkColumns: linkColumns.map((col) => ({
                  ...
                  fieldKey:
                    (col.field as { key?: string } | undefined)?.key ?? null,
    ...
    server.tool(
      ...
      'Copy a view from one Knack scene/page to another. Requires "allowViewMutation": true.',
      {
        ...
        sourceSceneKey: z
          .string()
          .describe("The source scene/page key that currently owns the view."),
        ...
        contents: [
          {
            ...
            text: JSON.stringify({ ok: false, message: "Unknown appKey" }),
          },
        ],
      };
    }
      ...
      contents: [
        {
          uri: uri.toString(),
          mimeType: "application/json",
          text: JSON.stringify({ ok: false, message: "Unknown resource type" }),
        },
      ],
    };
  });
