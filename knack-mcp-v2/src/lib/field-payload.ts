...
export const SCHEMA_CACHE_STALE_NOTE =
    'Schema cache not auto-invalidated — run knack_cache(refresh:true, warm:true) before trusting cached-schema tools.';
...
export const VIEW_CACHE_STALE_NOTE =
    'View cache not auto-invalidated — run knack_cache(refresh:true, warm:true) before trusting cached-view tools.';
...
export function validateEquationTokens(
    ...
    if (!object) {
        warnings.push(
            `Could not validate equation tokens: object ${objectKey} was not found in the cached schema, so this write is going out unchecked. Run knack_cache with refresh:true, warm:true and re-check if that is unexpected.`,
        );
