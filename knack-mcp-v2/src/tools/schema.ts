...
const FIELD_MAP_FILE_NOTE =
    'Resolved from the on-disk fieldMap.json cache. If a field was created, renamed, or deleted recently, this map may be stale until knack_cache is run with appKey set to this app plus refresh: true, warm: true, persistFiles: true. Without the appKey it refreshes every configured app.';
    ...
    handler: async ({ appKey, objectKey }, ctx) => {
        ...
        return makeTextResponse({
            ...
            note: connectionFields.some((c) => !c.connectedObjectKey)
                ? 'Some connection targets are unknown. Run knack_cache with appKey set to this app plus refresh:true, warm:true to load fresh runtime metadata which includes relationship details.'
                : null,
        });
    },
});
