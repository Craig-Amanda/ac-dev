...
| `knack_add_view_columns`          | view        | Appends new field columns to a table view's existing `columns`; reads the live array itself, so it never needs `allowDiagnostics` or the caller's own copy of the other columns         |
